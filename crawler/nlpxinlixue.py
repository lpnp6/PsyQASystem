...
class article:
    def __init__(self, title, link, content, token):
        self.title = title
        self.link = link
...
async def get_catalog(page_num):
    async with httpx.AsyncClient(verify=False) as client:
        ...
        for article_panel in selector.css("div.article-panel"):
            ...
            catalog.append(article(title, link, "", 0))
        return catalog
...
async def crawler():
    ...
    for i in range(1, page_num + 1):
        ...
        df = pd.DataFrame(columns=["ID", "Title", "Link", "Text", "Token"])
        for article in catalog:
            try:
                article.content = await get_article_content(article.link)
                article.token = len(encoder.encode(article.content))
                df.loc[len(df)] = [
                    id,
                    article.title,
                    article.link,
                ...
                with open("data/error.log", "a", encoding="utf-8") as f:
                    f.write(article.title + article.link + "\n")
        df.to_json(
            "data/nlpxinlixue.json",
            orient="records",
            mode="a",
            lines=True,
            force_ascii=False,
